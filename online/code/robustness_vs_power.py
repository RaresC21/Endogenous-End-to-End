...
if __name__ == '__main__':
    ...
    robustness_levels = [0.05, 0.1, 0.2]
    ...
    robust_rewards_all = [[] for _ in range(len(robustness_levels))]
    ...
    for power in powers:
        robust_rewards = [[] for i in range(len(robustness_levels))]
        robust_rewards_nn = []
        kernel_rewards = []
        baseline_rewards = []
        best_rewards = []

        demand_generator = Demand(n_features, power)
        ...
        for _ in tqdm(range(n_trials)): 
            ...
            for i, robustness in enumerate(robustness_levels): 
                ...
                robust_rewards[i].append(robust_action_ * demand_generator.get_mean_demand(robust_action_, test_feature))
        ...
        for i in range(len(robustness_levels)):
            robust_rewards_all[i].append(np.mean(robust_rewards[i]))
        ...
        print("robust:", robust_rewards_all[-1])
    ...
    plt.plot(powers, best_rewards_all, label = 'optimal', linestyle='--', color = 'green')
    plt.plot(powers, baseline_rewards_all, label = 'baseline', linestyle='-.', color = 'orange')
    # plt.plot(powers, kernel_rewards_all, label = 'kernel', linestyle=':', color = 'purple')
    for i in range(len(robustness_levels)):
        plt.plot(powers, robust_rewards_all[i], label = 'robust, eps={}'.format(robustness_levels[i]))

    plt.legend()
    plt.xlabel('exponent')
    plt.ylabel('Average reward')
    plt.title('Reward vs complexity of data')
    plt.show()
