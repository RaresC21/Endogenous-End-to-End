...
class SquareCB:
    ''' SquareCP first runs a linear regression for every action, then sample an action from a created probability distribution'''

    def __init__(self, actions, k, mu=20, gamma_scale=10, gamma_exp = 0.7):
        self.actions = actions
        self.k = k
        self.mu = mu
    ...
    def choose(self, agent):
        ...
        p = len(self.actions)
        self.t = agent.t
        past_actions  = np.array([self.actions[i] for i in agent.actions_list])
        # maximize the g function (demand * price)
        # features are prices and prices squared
        features = np.zeros((len(past_actions), 2))
        features[:, 0] = past_actions
        features[:, 1] = past_actions ** 2
        reg = LinearRegression().fit(features, agent.demands_list)
        # print(np.array(self.actions))
        # print(np.array(self.actions).reshape(-1, 1))
        # features with prices and prices squared
        to_predict = np.zeros((len(self.actions), 2))
        to_predict[:, 0] = self.actions
        to_predict[:, 1] = self.actions ** 2
        ...
        # sample action
        price = np.random.choice(self.actions, p=probs)
        action = np.where(self.actions == price)[0][0]
