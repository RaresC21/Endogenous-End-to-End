...
class Environment(object):
    ...
    def run(self, actions, trials=100, experiments=1):
        ...
        distance_to_optimal = np.zeros_like(scores)
        ...
        for k in tqdm(range(experiments)):
            ...
            for i, agent in enumerate(self.agents):
                if 'LinUCB' not in str(agent): # LinUCB doesn't need initialization
                    for action, price in enumerate(actions):
                        ...
                        distance_to_optimal[0, i] += abs(price - self.bandit.actions[self.bandit.optimal]) / len(actions)
                        if is_optimal:
            ...
            for t in range(1, trials):
                ...
                for i, agent in enumerate(self.agents):
                    ...
                    if 'LinUCB' not in str(agent):
                        ...
                        scores[t, i] += reward
                        distance_to_optimal[t, i] += abs(price - self.bandit.actions[self.bandit.optimal])
                        if is_optimal:
                            optimal[t, i] += 1
            ...
            for i, agent in enumerate(self.agents):
                if 'LinUCB' in str(agent):
                    ...
                    distance_to_optimal[:, i] += abs(np.array(list_actions) - self.bandit.optimal)
                    if k == 0:
        ...
        return scores / experiments, optimal / experiments, distance_to_optimal / experiments

    def plot_results(self, scores, optimal, distance_to_optimal):
        optimal = optimal * 100
        sns.set_style('white')
        ...
        # plt.legend(self.agents, loc=4)
        for i in range (distance_to_optimal.shape[1]):
            plt.plot(distance_to_optimal[:, i])
        plt.plot([0, len(scores)], [0] * 2, 'k--')
