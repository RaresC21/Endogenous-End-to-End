import os
import sys
from agent import AgentRobustLearning, NaiveAgent, AgentRobustLearningNN
from policy import RobustLearningPolicy, NaivePolicy, RobustLearningPolicyNN
...
def plot_results(scores, optimal, n_arms, mean, var_coeff, n_trials, n_experiments, agents_names, optimal_reward):
    ...
    plt.savefig('offline_learning/images/' + title + '.png', bbox_inches='tight')
...
def generate_plots(n_arms=10, mean='linear', var_coeff=1, n_trials=2000, n_experiments=500):
    _, _, actions, parameters = random_data(n_arms, mean, var_coeff)
    mu, sigma = zip(*parameters)

    bandit = GaussianBandit(n_arms, mu, sigma, actions)
    agents = [AgentRobustLearning(n_arms, RobustLearningPolicy(actions), actions), 
              AgentRobustLearningNN(n_arms, RobustLearningPolicyNN(actions), actions),
              NaiveAgent(actions, NaivePolicy(actions))
              ]
...
if __name__ == '__main__':
    ...
    for (n_arms, mean, var_coeff) in tqdm(params):
        score, optimal, optimal_reward, agent_names = generate_plots(n_arms, mean, var_coeff, 1, 100)
